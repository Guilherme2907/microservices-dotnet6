    ...
    public class RabbitMQMessageSender : IRabbitMQMessageSender
    {
        ...
        public void SendMessage(BaseMessage message)
        {
            if (ConnectionExists())
            {
                ...
                channel.BasicPublish(exchange: EXCHANGE_NAME, "PaymentEmail", basicProperties: null, body);
                channel.BasicPublish(exchange: EXCHANGE_NAME, "PaymentOrder", basicProperties: null, body);
            }
        }

        private byte[] GetMessageAsByteArray(BaseMessage message)
        {
            //Serialize the child class too
